...
|---|---|
| `credentials.test.ts` | Phase 2 #1–6 (login decision logic), rate limiting + Phase 8 #1, 2, 7, 8, 15, 23 (EMPLOYER authenticates by username only, never email even if one is on file; ADMIN email login unchanged; expired/no-accessExpiresAt EMPLOYER denied distinguishably only after credentials verify; SUSPENDED blocked regardless of expiry) |
| `username.test.ts` | Phase 8 #3 (username normalization — pure, no DB, no auth) |
| `tokens.test.ts` | Phase 2 #7, 8, 9, 10, 15 (token lifecycle, account setup, raw-token storage) |
| `authorize.test.ts` | Phase 2 #11, 12, 13, 14 (session/role authorization) + Phase 6 #1–5 (requireAdmin() boundary: logged-out, EMPLOYER, PENDING/SUSPENDED admin, ACTIVE admin) + Phase 8 #9 (an EMPLOYER's already-issued session — matching sessionVersion, ACTIVE status — is rejected the moment accessExpiresAt passes; ADMIN never subject to it) |
...
| `admin-clients-service.test.ts` | Phase 8: admin client service authorization boundary (every export rejects before touching Prisma — mocked) + #4, 5, 6 (duplicate username rejected without overwriting, password stored as a real bcrypt hash only, accessExpiresAt ≈ now + 72h) + #16, 17 (extend-from-current-expiry vs. reactivate-from-now math) + #18, 19, 25 (password reset hashes + bumps sessionVersion, audit row carries no password content) + #21, 22 (updateClientDetails has no username/accessExpiresAt/passwordHash parameter at all) — mocked Prisma, real bcrypt hashing |
| `admin-clients-integration.test.ts` | Phase 8 full lifecycle against real Postgres: Admin creates a client (#6) → client authenticates by username (#1), not email (#2) → unexpired client browses/views/shortlists (#7) → access is expired directly in the DB (never deleted) and the SAME already-issued session is denied across listing/profile/shortlist/biodata/photo (#8–14) → a fresh login now reports expired, distinguishably (#8) → SUSPENDED blocks login regardless of expiry (#15) → Admin reactivates a fully-expired client for 3 more days (#17) and login/access work again → Admin resets the password, invalidating the old one and the old session immediately (#18, 19) — **real database**, fictional fixtures only, everything deleted in `afterAll` |

`maids-integration.test.ts`, `shortlist-integration.test.ts`, `admin-maids-integration.test.ts`, `account-integration.test.ts`, and `admin-clients-integration.test.ts` are the files here that hit the actual `sgmaid-dev` database (via `tests/setup.ts` loading `.env`) instead of a mock — deliberately, per the Phase 3/4/6/7/8 spec's requirement to verify the real Prisma query/service layer, not just a mocked stand-in for it. Each creates its own throwaway test `User` row(s) (deleted in `afterAll`); `shortlist-integration.test.ts` additionally creates its own throwaway `MaidProfile` fixtures (Phase 6.2) rather than depending on the live status of shared seed data (as does `maids-integration.test.ts` since the fictional seed maids were removed); `admin-maids-integration.test.ts` additionally creates and deletes its own throwaway `MaidProfile` (and the one real Storage object it uploads); `admin-clients-integration.test.ts` likewise creates and deletes its own throwaway `MaidProfile` fixture. None leave any `MaidProfile` row, or any of the real pilot profiles, changed after the suite finishes.
